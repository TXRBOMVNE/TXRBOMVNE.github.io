...
export class EditTabService {
  ...
  addBar() {
    const addDefaultSegment = () => {
      let notesArray: Note[] = []
      for (let i = this.song.instrument.strings - 1; i > -1; i--) {
        notesArray.push({ string: i })
      }
      let defaultSegment = new Segment(false, 4, notesArray)
      return [defaultSegment]
    }
    this.song.bars.push(new Bar(
      this.song.bars[this.song.bars.length - 1].tempo,
      this.song.bars[this.song.bars.length - 1].timeSignature,
      addDefaultSegment()
    ))
  }

  private addSegment(barIndex: number) {
    this.song.bars[barIndex].segments.push(new Segment(this.notePropertiesInput.isRest, this.notePropertiesInput.initialDurationInverse, []))
  ...
  changeTimeSignature(timeSignatureNumerator: number, timeSignatureDenominator: number) {
    if (!this.noteSelection) return
    let barIndex = this.song.bars.indexOf(this.noteSelection.bar)
    this.song.bars[barIndex].timeSignature = { numerator: timeSignatureNumerator, denominator: timeSignatureDenominator }
  }
  ...
  changeSlides(slidesInput: { slideOut?: "slideUp" | "slideDown" | "slideToNote", slideIn?: "slideUp" | "slideDown" }) {
    ...
    let segmentIndex = this.song.bars[barIndex].segments.indexOf(this.noteSelection.segment)
    let noteIndex = this.song.bars[barIndex].segments[segmentIndex].notes!.indexOf(this.noteSelection.note!)
    if (!this.song.bars[barIndex].segments[segmentIndex].notes[noteIndex].effects) {
      this.song.bars[barIndex].segments[segmentIndex].notes[noteIndex].effects = {}
    }
    this.song.bars[barIndex].segments[segmentIndex].notes[noteIndex].effects!.slides = slidesInput
  }
  ...
  toggleRest(isRest: boolean) {
    ...
    this.song.bars[barIndex].segments[segmentIndex].isRest = isRest
  }
  ...
  selectNextSegmentNote() {
    ...
    if (this.noteSelection.note && !this.noteSelection.segment.isRest) {
      noteIndex = this.song.bars[barIndex].segments[segmentIndex].notes!.indexOf(this.noteSelection.note!)
    }
    ...
    if (this.noteSelection.bar.segments[segmentIndex] === this.noteSelection.bar.segments[this.noteSelection.bar.segments.length - 1]) {
      if (this.noteSelection.bar.valid) {
        if (!this.song.bars[barIndex + 1]) {
          this.addBar()
          this.HTMLBars?.changes.pipe(first()).subscribe(() => {
  ...
  selectPreviousSegmentNote() {
    ...
    if ((this.song.bars[barIndex] === this.song.bars[this.song.bars.length - 1] &&
      ...
      this.song.bars[barIndex].segments[segmentIndex].notes.every(note => !note.fretValue && note.fretValue !== 0)
    ) {
      // If there is only one segment, removes last bar; removes only segment otherwise
      if (this.song.bars[barIndex].segments.length <= 1) {
        this.song.bars.pop()
      } else {
    ...
    if (this.noteSelection.bar.segments[segmentIndex - 1]) {
      ...
      barHTML = this.HTMLBars?.get(barIndex - 1).el.nativeElement
      previousSegmentHTML = barHTML.lastElementChild.children[this.song.bars[barIndex - 1].segments.length - 1]
      if (this.noteSelection.note) {
        ...
        previousNoteHTML = previousSegmentHTML.lastElementChild.children[Math.floor(this.song.instrument.strings / 2)]
        previousNoteHTML.focus()
      }
    }
  }
  ...
  removeNote() {
    ...
    if (this.noteSelection.note) {
      ...
      if (this.noteSelection.segment.notes.every(note => !note.fretValue && note.fretValue !== 0)) {
        if (this.noteSelection.bar.segments.length === 1 && this.song.bars.length !== 1) {
          this.song.bars.splice(barIndex, 1)
        } else {
          this.song.bars[barIndex].segments.splice(segmentIndex, 1)
        }
        this.noteSelection = undefined
        return
      }
      this.song.bars[barIndex].segments[segmentIndex].notes[noteIndex].fretValue = undefined
    } else if (this.noteSelection.segment.isRest) {
      if (this.noteSelection.bar.segments.length === 1 && this.song.bars.length !== 1) {
        this.song.bars.splice(barIndex, 1)
      } else {
  ...
  fillSegmentsNotesSpaces() {
    this.song.bars.forEach(bar => {
      bar.segments.forEach(segment => {
        ...
        for (let i = 0; i < this.song.instrument.strings; i++) {
